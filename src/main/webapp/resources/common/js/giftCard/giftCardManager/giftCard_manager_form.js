$(document).ready(function() {
	...
	// 상품권관리 추가 기능
	$("#giftC_detail_add").click(function() {
		var $form = $('<form></form>');
	...
	// 상품권관리 편집 기능
	$("#giftcard_manager_detail_mdfy").click(function() {
		$("#giftbon_insert_btn_div").hide();
	...
	// 상품권 등록 저장
	$("#giftcard_manager_addsave").click(function() {
	...
	// 상품권 추가 취소 기능
	$("#giftcard_manager_acancel").click(function() {
...
/* 팝업창 가맹점목록 표시 */
function viewProdMenuList(prodMenuPageNum, schDiv) {
	var ctx = $("#ctx").val();
	...
	$.ajax({
		...
		success: function(data) { 
			...
			if(data.prodMenuPage.endPageNum == 0 || data.prodMenuPage.endPageNum == 1){
				...
				+"<a onclick=\"viewProdMenuList("+data.prodMenuPage.endPageNum+", 2);\" id='pNum' style='cursor: pointer;'> / "+data.prodMenuPage.endPageNum+"</a>"
				+"<a onclick=\"viewProdMenuList("+(data.prodMenuPageNum+1)+", 2);\" id='pNum' style='cursor: pointer;'> ▶ </a>";
	...
	$(document).ready(function() {
		...
		if (keycode == '13') {
			...
			if ($("#prodMenuPageInput").val() == '') {
				...
				viewProdMenuList(prodMenuPageNum, 2);
			}
		}
		event.stopPropagation();
	});
