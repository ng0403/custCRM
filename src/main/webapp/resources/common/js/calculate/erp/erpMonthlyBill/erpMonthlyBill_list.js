/**
업 무 명 : ERP 가맹점별 월수수료 정산 화면
작 성 자 : 정은지 (wjdmwi@coreplus.co.kr)
작 성 일 : 2015/09/14
수 정 자 : 정은지 (wjdmwi@coreplus.co.kr)
수 정 일 : 2015/09/14
내 용 : ERP 가맹점별 월수수료 정산에 대한 javascript 코드이다.
*참고사항 : 
*/

//ERP 가맹점별 월수수료 정산 관리 페이징
function EBPaging(pageNum) {
	$(document).ready(function() {
		var ctx = $("#ctx").val();
		var $form = $('#erpMonthlyBillPagingForm');
...
function erpBill_goSearch(){
	...
	viewLoadingShow();
	$("#monthlyBillForm").submit();
}

//페이지 엔터키 기능
function erpbillpageNumEnter(event) {
	$(document).ready(function() {
		...
		if (keycode == '13') {
			...
			if ($("#pageInput").val() == '') {
				alert("페이지 번호를 입력하세요.")
				$("#pageInput").val($("#erpBillPageNum").val());
				$("#pageInput").focus();
...
function go_erpBill_list() {
	var ctx = $("#ctx").val();
	
	// 동적 폼생성 get 전송
	var $form = $('<form></form>');
	$form.attr('action', ctx+'/erpMonthlyBill');
